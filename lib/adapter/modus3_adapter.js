...
class Modus3Adapter extends PrinterAdapter {
  ...
  static getHandle () {
    const handle = cptWrapper.getXmlPrintHandle(Buffer.from('CUSTOM_SPA_MODUS3'))
    // if (handle.deref() === null) {
    //   throw new PrinterError('getHandle', printErrors.codes.CUSTOM_XMLPRINT_GENERIC_FAILURE)
    // }
    return handle
  }
  ...
  static setXmlTagValue (handle, tag, value) {
    const result = cptWrapper.setXmlObjectTag(handle, Buffer.from(tag), Buffer.from(value, 'utf8'))
    if (result !== printErrors.codes.CUSTOM_XMLPRINT_SUCCESS) {
