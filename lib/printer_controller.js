...
const StatusManager = require('lib/status_manager')
...
class PrinterController extends EventEmitter {
  ...
  _statusManagerEvents () {
    this.statusManager.on('statusChanged', (status) => {
      log('printer status: ', statusParser.toStringArray(statusParser.parse(rawStatus)))
    })
    this.statusManager.on('printerDisconnected', (error) => {
      log('printer status: ', this.statusError.toString(error.code))
      try {
  ...
  _closeStatus () {
    try {
      ...
      _assertPrintError(error)
      log('printer close_error', this.statusError.toString(error.code))
    }
  }
